...
while start_time < end_time:
    ...
    # Download the file
    file_name = download_CMI(yyyymmddhhmn, '08', input)
    file_name2 = download_CMI(yyyymmddhhmn, '15', input)
    ...
    # Plot the image
    img3 = ax.imshow(datafinal, origin='upper', extent=img_extent, vmax=vmax, vmin=vmin, cmap=colormap)
...
#    plt.title('Reg.: ' + str(extent), fontsize=10, loc='right')
    ...
    filename = start_time.strftime('%H%M') + '.png'
    plt.savefig(f'{output}/profundidade_zoom_{filename}', bbox_inches='tight', pad_inches=0.2, dpi=300)
